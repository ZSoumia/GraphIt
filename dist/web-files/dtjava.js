...
var dtjava = function() {
    ...
    function addOnload(fn) {
        if (isDef(w.addEventListner)) {
            w.addEventListner("load", fn, false);
        } else if (isDef(d.addEventListner)) {
            d.addEventListner("load", fn, false);
        } else if (isDef(w.attachEvent)) {
            w.attachEvent("onload", fn);
            //TODO: swfobject also keeps references to the listners to detach them on onload
   ...
   function showMessageBox() {
    ...
    function init() {
        ...
        if (!cbDone) {
            if (isDef(d.addEventListner)) {
                d.addEventListner("DOMContentLoaded",
            ...
            if (ua.ie && ua.win) {
                ...
                if (isDef(d.addEventListner)) {
                    d.addEventListner("onreadystatechange", function() {
                        if (d.readyState == "complete") {
                            d.removeEventListner("onreadystatechange", arguments.callee, false);
                            invokeCallbacks();
                        }
                    }, false);
                } else {
...
//            lnk.style.marginLeft = -77; //155/2
    ...
    return {
        ...
        /**
         ...
         requirements listd here will NOT be validated prior to launch
