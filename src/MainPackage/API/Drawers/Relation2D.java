...
public class Relation2D extends Line implements Drawable, Comparable{
    ...
    public Relation2D(Relation relation) throws NoVertex2DException {
        Vertex2D sg = relation.getLeftVertex().getVertex2D();
        Vertex2D sd = relation.getRightVertex().getVertex2D();
                      
        if(sd == null || sg == null) 
            throw new NoVertex2DException();
    ...
    public void setVertex(Relation relation){
        this.relation.setValue(relation);
    }
    ...
    private void initLabelBindings(){
        ...
        this.label2D.textProperty().bind(relation.labelProperty().concat(
            (relation instanceof WeightedEdge ? ": " + ((WeightedEdge) relation).getWeight() : (
                    relation instanceof WeightedArc ? ": " + ((WeightedArc) relation).getWeight() : ""))));
    ...
    private void initComponentsBindings(){
        // Layout shouldn't influence vectors' components        
        this.layoutXProperty().addListener(event -> this.setLayoutX(0.0));
        this.layoutYProperty().addListener(event -> this.setLayoutY(0.0));
        //
        
        Vertex2D sg = this.getRelation().getLeftVertex().getVertex2D();
        Vertex2D sd = this.getRelation().getRightVertex().getVertex2D();
    ...
    private synchronized void updateComponents(Vertex2D sg, Vertex2D sd){
        double dx = this.getRelation().getRightVertex().getVertex2D().getCenterX() 
                    - 
                    this.getRelation().getLeftVertex().getVertex2D().getCenterX();
    ...
    public String UIBehavior() {
        return this.getRelation().getClass().getSimpleName() + ": " + this.getRelation().getLabel() + 
                (this.getRelation().getWeightValue().getValue() instanceof WeightValue ? " [" + this.getRelation().getWeightValue() + "]" : "");
    }
    ...
    public class Arrow {
        ...
        private void updateArrowsComponents(){
            ...
            double dy = this.getRelation().getRightVertex().getVertex2D().getCenterY() 
                        - 
                        this.getRelation().getLeftVertex().getVertex2D().getCenterY();
            ...
            if(dx == 0){
                ...
                // Calculation of the point P coordinates 
                ...
                // Calculation of point w' coordinates 
